...
public class Board
{
    ...
    private void CreateBoard()
    {
        ...
        GameObject prefabBG = /*Resources.Load<GameObject>(Constants.PREFAB_CELL_BACKGROUND)*/ GameAssets.Instance.backgroundCellPrefab;
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                GameObject go = GameObject.Instantiate(prefabBG);
                //GameObject go = Lean.Pool.LeanPool.Spawn(prefabBG);
    ...
    public void Clear()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x, y];
                cell.Clear();
                //Lean.Pool.LeanPool.Despawn(cell.gameObject);
