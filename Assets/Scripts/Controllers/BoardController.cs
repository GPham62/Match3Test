...
public class BoardController : MonoBehaviour
{
    ...
    public bool IsBusy()
    {
        return m_gamePlayState == GamePlayState.Pause;
    }
    ...
    private void OnGameStateChange(GameManager.eStateGame state)
    {
        switch (state)
        {
            case GameManager.eStateGame.GAME_STARTED:
                m_gamePlayState = GamePlayState.Idle;
                //IsBusy = false;
                break;
    ...
    public void Update()
    {
        if (m_gamePlayState == GamePlayState.Pause || m_gamePlayState == GamePlayState.GameOver) return;
        if (!m_hintIsShown && m_gamePlayState == GamePlayState.Idle)
        {
        ...
        if (m_gamePlayState == GamePlayState.Idle)
        {
            if (Input.GetMouseButtonDown(0))
            {
                if (CheckRaycastHit(out var collider))
                {
                    m_hitCollider = collider;
                    m_gamePlayState = GamePlayState.SelectingMatch;
                }
            }
        }
        else if (m_gamePlayState == GamePlayState.SelectingMatch)
        {
            if (Input.GetMouseButton(0))
            {
                if (CheckRaycastHit(out var collider))
                {
                    if (m_hitCollider != collider)
                    {
                        StopHints();
                        Cell c1 = m_hitCollider.GetComponent<Cell>();
                        Cell c2 = collider.GetComponent<Cell>();
                        if (AreItemsNeighbor(c1, c2))
                        {
                            m_gamePlayState = GamePlayState.Animating;
                            ...
                            ///TODO: Duplicate call inside FindMatchesAndCollapse, need to revisit later
                            m_gamePlayState = GamePlayState.Idle;
                        }
                        else
                        {
    ...
    private bool CheckRaycastHit(out Collider2D collider)
    {
        var hit = Physics2D.Raycast(m_cam.ScreenToWorldPoint(Input.mousePosition), Vector2.zero);
